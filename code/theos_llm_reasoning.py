...
from llm_adapter import LLMAdapter, LLMResponse, get_llm_adapter
from semantic_retrieval import MockEmbeddingAdapter, SemanticRetrieval
...
class THEOSLLMReasoner:
    ...
    def reason(self, query: str) -> THEOSResult:
        ...
        similar_wisdom = self.retrieval.retrieve_similar(
            query, threshold=self.similarity_threshold, top_k=3
        )
        ...
        for cycle_num in range(1, self.max_cycles + 1):
            ...
            contradiction = self._measure_contradiction(
                constructive_response.content, critical_response.content
            )
            ...
            observation = self._blend_outputs(
                constructive_response.content, critical_response.content, contradiction
            )
            ...
            halt_reason = self._check_halt_criteria(
                cycle_num=cycle_num, contradiction=contradiction, quality=quality, cycles=cycles
            )
