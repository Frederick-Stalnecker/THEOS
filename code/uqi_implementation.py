...
class UnifiedResult:
    ...
    def __post_init__(self):
        ...
        assert self.total_count >= len(self.records), "Total count must be >= returned records"
...
class JSONBackend(BackendInterface):
    ...
    def _build_index(self) -> dict:
        ...
        index = {"by_domain": {}, "by_question_hash": {}, "by_tags": {}}
    ...
    def query(self, query: UnifiedQuery) -> UnifiedResult:
        ...
        for record in candidates:
            similarity = self._compute_similarity(query.query_text, record["question"])
...
class EscalationManager:
    ...
    def check_escalation(self) -> bool:
        ...
        if (
            self.current_backend == StorageBackend.JSON
            and record_count > self.JSON_TO_SQLITE_THRESHOLD
        ):
            ...
            and record_count > self.SQLITE_TO_VECTOR_DB_THRESHOLD
        ):
...
class UnifiedWisdomInterface:
    ...
    def query_wisdom(
        self, domain: str, question: str, similarity_threshold: float = 0.85, max_results: int = 10
    ) -> UnifiedResult:
        """
...
if __name__ == "__main__":
    ...
    results = wisdom.query_wisdom(
        domain="medical", question="What are symptoms of pneumonia?", similarity_threshold=0.85
    )
